...
namespace ScottPlot2
{
    public class ScottPlot
    {
        ...
        public int dataSizeX, dataSizeY, dataPointCount, dataLinesCount;
        ...
        public string Info()
        {
            ...
            info += String.Format("The last data addition loaded {0} points into memory.\n", dataPointCount.ToString("N0"));
            info += String.Format("The last rendering drew {0} lines.\n", dataLinesCount.ToString("N0"));
        ...
        public void AddLineXY(List<double> Xs, List<double> Ys, bool drawGrid = true, bool skipPointsOutsideAxis = true)
        {
            if (Ys == null)
            {
            ...
            if (AX.GetAxis()[0] == 0 && AX.GetAxis()[1] == 0) { AX.Auto(Xs, Ys); }
            if (drawGrid) DrawGrid();
            List<Point> points = new List<Point>();
            for (int i = 0; i < Xs.Count; i++)
            {
                if ((skipPointsOutsideAxis==true)&&((Xs[i] < AX.GetAxis()[0]) || Xs[i] > AX.GetAxis()[1])) continue;
            ...
            dataPointCount = Ys.Count(); // so we have it for later
            if (points.Count() < 2) return;
            if (highQuality==true) graphicsData.SmoothingMode = System.Drawing.Drawing2D.SmoothingMode.AntiAlias;
            else graphicsData.SmoothingMode = System.Drawing.Drawing2D.SmoothingMode.HighSpeed;
        ...
        public void AddLineSignal(List<double> Ys, double unitsPerPoint = 10, double offsetX = 0,  bool drawGrid = true)
        {
            ...
            for (int x = 0; x < dataSizeX; x++)
            {
                ...
                //graphicsData.DrawLine(new Pen(Color.Red, 1), new Point(x, (int)colMin), new Point(x, (int)colMax));
        ...
        public void DrawAxis()
        {
            ...
            graphicsFigure.SmoothingMode = System.Drawing.Drawing2D.SmoothingMode.AntiAlias;
